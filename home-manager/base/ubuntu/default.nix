...
mkModule config ./. {
  ...
  # systemd's user manager starts with no XDG_DATA_DIRS, so a bare ''${XDG_DATA_DIRS}
  # expands to nothing and drops /usr/share — GNOME then finds no GSettings
  # schemas, gnome-session dies, and GDM loops back to the login screen.
  xdg.configFile."environment.d/99-nix-path.conf".text = ''
    PATH=${homeDir}/.nix-profile/bin:/nix/var/nix/profiles/default/bin:''${PATH}
    XDG_DATA_DIRS=${homeDir}/.nix-profile/share:/nix/var/nix/profiles/default/share:''${XDG_DATA_DIRS:-/usr/local/share:/usr/share}
  '';
}
